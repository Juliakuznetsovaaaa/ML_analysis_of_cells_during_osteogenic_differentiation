...
def hist(df1, df2, name, title):
    data1 = df1[name][df1[name] <= 5000].dropna()
    data2 = df2[name][df2[name] <= 5000].dropna()
    ...
    plt.hist(data1, weights=weights1, alpha=0.9, label='Больной')
    plt.hist(data2, weights=weights2, alpha=0.9, label='Здоровый')
...
if __name__ == '__main__':
        file1='merged_file_od.csv'
        file2='merged_file_cntrl.csv'
